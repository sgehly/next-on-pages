...
var transform = async ({
  ...
  for (const [name, filepath] of functionsMap) {
    ...
    for (const entry of functionsEntries) {
      if (`pages/${name}` === entry?.name || `app${name !== "index" ? `/${name}` : ""}/page` === entry?.name) {
        hydratedFunctions.set(name, { matchers: entry.matchers, filepath });
      }
    }
  }
  const rscFunctions = [...functionsMap.keys()].filter(
    (name) => name.endsWith(".rsc")
  );
  if (hydratedMiddleware.size + hydratedFunctions.size !== functionsMap.size - rscFunctions.length) {
    console.error(
...
var generateRoutes = async () => {
  try {
    await (0, import_promises.writeFile)(
      ".vercel/output/static/_routes.json",
      JSON.stringify({
        version: 1,
        description: `Built with @cloudflare/next-on-pages@${version2}.`,
        include: ["/*"],
